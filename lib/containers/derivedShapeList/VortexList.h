...
#include "../ShapeList.h"
#include "../../objects/Vortex.h"
#include "../../Direction.h"
#include "../../ShapeType.h"

class VortexList: public ShapeList{
protected:
	std::list<Vortex> vortexes;
	...
	virtual void make(olc::vi2d _loc, Direction dirCode) override {
		vortexes.push_back(Vortex(*pixelGameEngine, *animationContainer, _loc, default_speed, default_color, default_pedal_color, default_radius, dirCode));
	}

public:
	VortexList()
	: ShapeList(ShapeType::OBJECT){}
	VortexList(olc::PixelGameEngine& pge, AnimationContainer& ac, olc::Pixel _color, olc::Pixel _pedal_color, uint8_t _speed, uint8_t _radius)
	: ShapeList(pge, ac, ShapeType::OBJECT), default_color(_color), default_pedal_color(_pedal_color), default_speed(_speed), default_radius(_radius){}
	...
	virtual void moveAll() override {
		for(auto it = vortexes.begin(); it != vortexes.end();){
			it->move();
			if(it->outOfBounds()){
				vortexes.erase(it++);
			}else{
	...
	virtual int checkCollisionsWith(ShapeList& list, bool removeOnCollision = true) override {
		for(auto it = vortexes.begin(); it != vortexes.end(); ++it){
			if(list.checkCollisionsWith(*it) > 0){
				...
				if(removeOnCollision) vortexes.erase(it);
				return radius;
			}
		}
